...
* Phase 5 ✅ Performance sync — XLSX upload + normalization, UTM matching, dashboard with date/platform/type filters, summary cards, per-platform metrics, line+bar charts, sortable posts table, AI insights panel. LinkedIn is fully functional. Instagram, Facebook, X/Twitter, and YouTube parsers are stubbed but disabled (marked "coming soon" in UI) pending platform-specific export debugging. LinkedIn-specific fixes applied: reads from the "All posts" sheet (not "Metrics"), skips the prose description in row 1 and uses row 2 as the real header row, converts engagement rate and CTR from decimal to percentage (LinkedIn exports 0.226 = 22.6%). Duplicate detection with override/skip prompt. Import summary panel.
* Phase 6 ✅ Design handoff — external tool integration at https://design-dog.vercel.app/. Content Studio "Design" sidebar section shows the selected brief's copy per platform in read-only fields with one-click copy buttons. "Open Design Tool" button opens the external tool in a new tab and logs the session (timestamp, briefId, platform) to data/mockups.json. "Create graphic →" link in Process Request results panel navigates directly to the Design section with the brief pre-selected via URL params (?section=design&briefId=). No canvas editor or html2canvas — visual creation handled externally.
* Phase 7 ✅ Proactive post suggestions — "Suggested for you" panel in Brainstorm Content section above starter prompts. 3-5 AI-generated post idea cards per batch (contentType, platforms, pillar, rationale, suggestedAngle, targetAudience, suggestedCampaign). "Open as Brief" navigates to Process Request with suggestion pre-filled. "Try in chat" populates the brainstorm chat input. "Dismiss ×" removes card and logs to KV so Claude doesn't resurface it. "Refresh suggestions" forces a new batch via POST. Batch cached in KV ('suggestions' key) for 24h — no redundant API calls on every page load. Dismissed log stored in KV ('dismissed-suggestions' key, last 100). Engine (pages/api/suggestions/generate.js) synthesises: full brand context from brand-settings.json, pillar coverage gap analysis from last 30 days of briefs, top-performing content types from performance data if available, last 10 brief titles (recency filter), recently dismissed suggestions. Falls back gracefully when no performance data exists — shows subtle notice. API route also exposes POST for forced refresh. Dismiss endpoint at pages/api/suggestions/dismiss.js.
* Phase 8 ✅ Product knowledge base — PDF/text document upload and ingestion in Brand Settings (Knowledge Base section, admin only). Documents are chunked into ~500-token segments with 50-token overlap and stored in Vercel KV (kb-index for doc metadata, kb-chunks-{id} for chunk arrays). Keyword-based search (TF-IDF term frequency scoring) at /api/knowledge/search — no vector embeddings needed. Knowledge retrieval is wired into all three AI call sites: /api/generate-copy (top 3 chunks by brief+audience+clouds query), /api/chat (top 2 chunks by last user message), /api/suggestions/generate (top 3 chunks by underrepresented pillar topics). System prompt injection under PRODUCT KNOWLEDGE CONTEXT section. UI indicators: "📚 Knowledge base used" badge on copy results with source doc names; "📖 Sources: ..." line on brainstorm chat assistant messages. Brand Settings overview has KB status card (doc count, chunk count, last upload, Manage link). Storage health warning shown when > 500 chunks. Pre-load by uploading PDFs through the UI — 5 document types supported: customer-story, product-portfolio, roadmap, playbook, strategy, other.
