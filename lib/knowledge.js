...
// Document types that get a relevance score boost (more useful for grounded copy)
const TYPE_BOOST = { 'customer-story': 1.5, 'product-portfolio': 1.5 }

/**
 * Score an array of chunk objects by keyword relevance to a query.
 * Score is normalized by query term count and chunk length so that
 * scores are comparable across chunks of different sizes.
 * Type boost applied for high-value doc types.
 * Returns top N chunks with score >= minScore, sorted descending.
 */
export function scoreChunks(chunks, query, topN = 5, minScore = 0.1) {
  const terms = (query || '')
    .toLowerCase()
    .match(/[a-z0-9]+/g)
  ...
  return chunks
    .map((chunk) => {
      ...
      for (const term of terms) {
        let p = 0
        while ((p = text.indexOf(term, p)) !== -1) { rawScore++; p += term.length }
      }
      // Normalize: average mentions per term per 500 chars of chunk length
      const norm = rawScore / (terms.length * Math.max(1, text.length / 500))
      const boost = TYPE_BOOST[chunk.docType] || 1
      const score = norm * boost
      return { ...chunk, score }
    })
